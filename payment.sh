...
echo -e ${paytables[@]}
echo ""
if [ $(date -d "$D" '+%d') -gt 25 ];then



echo -e ${paytables[@]} >> /home/Lawrence/git/hours/records/$(date -d '10 days' "+%m-20%y")-paytables
echo "$totalHours" >> /home/Lawrence/git/hours/records/$(date -d '+10 days' "+%m-20%y")-hours
runningTotal=$(echo $(awk '{s+=$1} END {print s}' /home/Lawrence/git/hours/records/$(date -d '10 days' "+%m-20%y")-hours))
else

	echo -e ${paytables[@]} >> /home/Lawrence/git/hours/records/$(date  "+%m-20%y")-paytables
#touch /home/Lawrence/git/hours/records/$(date -d "$D" '+%m')-$(date -d "$D" '+%Y')
	echo "$totalHours" >> /home/Lawrence/git/hours/records/$(date  "+%m-20%y")-hours
runningTotal=$(echo $(awk '{s+=$1} END {print s}' /home/Lawrence/git/hours/records/$(date  "+%m-20%y")-hours))
fi
